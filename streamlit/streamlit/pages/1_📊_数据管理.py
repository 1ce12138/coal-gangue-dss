...
with tab2:
    ...
    with st.form("manual_scheme_form", clear_on_submit=False):
        ...
        scheme_desc = st.text_area(
            "方案描述",
            placeholder="说明原料来源、处理路径、产品形态、建设条件、预期目标等。",
            height=150,
        )
        c1, c2 = st.columns(2)
        with c1:
        ...
        risk_note = st.text_area(
            "主要约束与风险",
            placeholder="可填写市场需求、环保约束、政策要求、二次污染、资金压力等。",
            height=90,
        )
    ...
    if submitted:
        if not scheme_desc.strip() and not scheme_name.strip():
            ...
            if current_df is None or current_df.empty:
                ...
                updated_df = pd.concat([current_df, new_row], ignore_index=True, sort=False)
