...
pair_df = consistency_df[[f"人工_{selected_dim}", f"AI_{selected_dim}"]].apply(
    pd.to_numeric, errors="coerce"
).dropna()

if len(pair_df) < 3:
    st.warning("当前维度有效成对样本不足 3 组，无法进行相关性检验。请补充数据或选择其他维度。")
    st.stop()

human = pair_df[f"人工_{selected_dim}"].values
ai = pair_df[f"AI_{selected_dim}"].values
metrics = calculate_consistency(human, ai)

st.markdown(f"#### {selected_dim} — 一致性指标")
st.caption(f"有效成对样本量 n = {metrics.get('n', len(human))}")
...
if len(available) > 1:
    ...
    summary = []
    for d in available:
        d_pair_df = consistency_df[[f"人工_{d}", f"AI_{d}"]].apply(
            pd.to_numeric, errors="coerce"
        ).dropna()
        h = d_pair_df[f"人工_{d}"].values
        a = d_pair_df[f"AI_{d}"].values
