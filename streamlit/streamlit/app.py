...
with col1:
    st.markdown(
        ...
        <div class="info-card">
            ...
            <div class="card-desc">
                在煤基固废管理领域提出基于 RAG 驱动的 LLM-as-a-Judge 评估范式，
            克服通用大语言模型在垂直专业领域知识推理的幻觉问题。
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
with col2:
    st.markdown(
        """
        <div class="info-card">
            <div class="card-title">多属性决策方法融合</div>
            <div class="card-desc">
                将层次分析法(AHP)等多准则决策方法深度内化到大模型推理的结构化提示模板中，
                保障量化评估结果的理论支撑与可解释性。
            </div>
        </div>
        """,
...
with col3:
    st.markdown(
        """
        <div class="info-card">
            <div class="card-title">全链路系统落地与验证</div>
            <div class="card-desc">
                构建端到端决策支持系统(DSS)，并通过大样本多统计学指标进行人机一致性联合检验，
                验证模型在实际管理决策场景下的可靠性。
            </div>
        </div>
        """,
...
with m3:
    render_metric_card("MiniMax", "大模型基座")
with m4:
    render_metric_card("RAG", "知识库增强引擎")
